  ...
  return (
    <div>
      <div
        ...
        style={{ border: `2px dashed ${isDragOver ? '#FF6B35' : 'rgba(255,107,53,0.35)'}`, borderRadius: '16px', padding: '48px 24px', textAlign: 'center', cursor: 'pointer', background: isDragOver ? 'rgba(255,107,53,0.08)' : 'rgba(255,255,255,0.03)', transition: 'all 0.2s' }}
      >
        <div style={{ fontSize: '40px', color: isDragOver ? '#FF6B35' : 'rgba(255,107,53,0.4)', marginBottom: '12px' }}>
          <i className="ti ti-file-text" aria-hidden="true" />
        </div>
        <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '18px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>Part 7 지문과 문제를 올리세요</div>
        <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', marginBottom: '16px' }}>지문 + 문제 사진을 드래그하거나 클릭해서 업로드</div>
      ...
      {fileName && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '14px 18px', marginTop: '14px' }}>
          ...
          <button onClick={() => { setFileName(null); setPreviewSrc(null); setImageBase64(null); setFileSize(null); setCompressedSize(null); if (fileRef.current) fileRef.current.value = '' }}
      ...
      <button
        ...
        disabled={!canAnalyze}
        style={{ width: '100%', marginTop: '16px', padding: '18px', background: canAnalyze ? '#FF6B35' : 'rgba(255,255,255,0.08)', border: canAnalyze ? 'none' : '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', color: canAnalyze ? '#fff' : 'rgba(255,255,255,0.25)', fontFamily: "'Space Grotesk', sans-serif", fontSize: '16px', fontWeight: 700, cursor: canAnalyze ? 'pointer' : 'not-allowed', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}
      >
