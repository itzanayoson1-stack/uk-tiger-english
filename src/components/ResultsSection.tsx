        ...
        <Card label="Card 5" title={isMulti ? 'Structure Mode (지문 간 흐름)' : 'Structure Mode (전개)'} emoji="🏗️">
          <div style={{ marginTop: '12px' }}>
            {result.structure_steps.map((s, i) => (
              <div key={i}>
                <div style={{ padding: '12px 16px', borderRadius: '10px', background: '#fff8f5', borderLeft: `3px solid ${isMulti ? '#16a34a' : '#FF6B35'}`, margin: '5px 0' }}>
                ...
                {i < result.structure_steps.length - 1 && <div style={{ textAlign: 'center', color: isMulti ? '#16a34a' : '#FF6B35', fontSize: '18px', margin: '2px 0', opacity: 0.5 }}>↓</div>}
              </div>
            ))}
          </div>
        </Card>
